...
// Open http://127.0.0.1:4777/, set the Jev key in settings, and send Jev requests to http://127.0.0.1:4777/v1/systemone
...
server.listen(port, "127.0.0.1", () => console.log(JSON.stringify({ viewer: `http://127.0.0.1:${port}/`, send_jev_requests_to: `http://127.0.0.1:${port}/v1/systemone`, jev_key: store.setting("jevKey") ? "set" : "not set: add it in the viewer's settings", database: store.database, recorded: store.summaries.length })));
